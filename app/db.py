...
- app.logger: To log events for debugging and monitoring.
"""
...
# Create a app.logger object
#app.logger = app.logger.getapp.logger(__name__)
...
def init_db():
    ...
    **Warning**: This function drops the existing 'protections' table, which will **delete all existing records**.
    ...
    try:
        ...
        app.logger.info("Initializing protections database.")
        ...
        app.logger.debug("Dropping existing 'protections' table if it exists.")
        cursor.execute('DROP TABLE IF EXISTS protections')
        
        # Create the protections table with specified columns
        app.logger.debug("Creating 'protections' table.")
        cursor.execute('''
        ...
        conn.commit()
        app.logger.info("'protections' table created successfully.")
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during init_db: {e}")
    except Exception as e:
        app.logger.error(f"Unexpected error during init_db: {e}")
    finally:
        conn.close()
        app.logger.debug("Database connection closed after init_db.")
...
def init_db_for_generated_files():
    ...
    **Warning**: This function drops the existing 'generated_files' table, which will **delete all existing records**.
    ...
    """
    try:
        app.logger.info("Initializing generated_files database.")
        ...
        app.logger.debug("Dropping existing 'generated_files' table if it exists.")
        cursor.execute('DROP TABLE IF EXISTS generated_files')
        
        # Create the generated_files table with specified columns
        app.logger.debug("Creating 'generated_files' table.")
        cursor.execute('''
        ...
        # Commit the changes and close the connection
        conn.commit()
        app.logger.info("'generated_files' table created successfully.")
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during init_db_for_generated_files: {e}")
    except Exception as e:
        app.logger.error(f"Unexpected error during init_db_for_generated_files: {e}")
    finally:
        conn.close()
        app.logger.debug("Database connection closed after init_db_for_generated_files.")
...
def load_csv_to_db():
    ...
    """
    try:
        app.logger.info("Starting to load CSV data into protections database.")
        ...
        with open(csv_path, mode='r') as file:
            ...
            for line_number, line in enumerate(csv_file, start=2):  # Start at 2 considering header
                ...
                if not line.get('ProtectionName') or not line.get('Resource'):
                    app.logger.warning(f"Skipping row {line_number}: Missing ProtectionName or Resource.")
                    continue
        
                # Only include certain services, e.g., exclude non-HTTP
                if line.get('Service', '').lower() != "http":
                    app.logger.debug(f"Skipping row {line_number}: Service is not 'http'.")
                    continue
                ...
                try:
                    ...
                    app.logger.debug(f"Inserted row {line_number} into 'protections' table.")
                except sqlite3.Error as e:
                    app.logger.error(f"SQLite error on row {line_number}: {e}")
                except Exception as e:
                    app.logger.error(f"Unexpected error on row {line_number}: {e}")
        
            # Commit the transactions and close the connection
            conn.commit()
            app.logger.info("CSV data loaded successfully into protections database.")
    except Exception as e:
        app.logger.error(f"Unexpected error during load_csv_to_db: {e}")
    finally:
        try:
            conn.close()
            app.logger.debug("Database connection closed after load_csv_to_db.")
        except:
...
def load_protections():
    ...
    """
    try:
        app.logger.info("Loading all protections from the database.")
        conn = sqlite3.connect('app/data/protections.db')
        cursor = conn.cursor()
        ...
        app.logger.debug(f"Fetched {len(data)} records from 'protections' table.")
    
        conn.close()
        app.logger.debug("Database connection closed after load_protections.")
    
        return data
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during load_protections: {e}")
        return []
    except Exception as e:
        app.logger.error(f"Unexpected error during load_protections: {e}")
        return []
...
def get_protection_by_name(protection_name):
    ...
    """
    try:
        app.logger.info(f"Fetching protection by name: {protection_name}")
        conn = sqlite3.connect('app/data/protections.db')
        cursor = conn.cursor()
        ...
        conn.close()
        app.logger.debug(f"Protection fetched: {result}")
        ...
        if result:
            ...
            app.logger.info(f"Protection '{protection_name}' retrieved successfully.")
            return protection
        else:
            app.logger.warning(f"Protection '{protection_name}' not found in the database.")
            return None
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during get_protection_by_name: {e}")
        return None
    except Exception as e:
        app.logger.error(f"Unexpected error during get_protection_by_name: {e}")
        return None
...
def save_generated_file_to_db(
    ...
    """
    try:
        app.logger.info(f"Saving generated file to database: {filename}")
        conn = sqlite3.connect(FILES_DB)
        cursor = conn.cursor()
        ...
        conn.commit()
        app.logger.info(f"Generated file '{filename}' saved successfully.")
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during save_generated_file_to_db: {e}")
    except Exception as e:
        app.logger.error(f"Unexpected error during save_generated_file_to_db: {e}")
    finally:
        conn.close()
        app.logger.debug("Database connection closed after save_generated_file_to_db.")
...
def load_generated_files():
    ...
    """
    try:
        app.logger.info("Loading all generated files from the database.")
        conn = sqlite3.connect(FILES_DB)
        cursor = conn.cursor()
        ...
        app.logger.debug(f"Fetched {len(files)} records from 'generated_files' table.")
    
        conn.close()
        app.logger.debug("Database connection closed after load_generated_files.")
    
        return files
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during load_generated_files: {e}")
        return []
    except Exception as e:
        app.logger.error(f"Unexpected error during load_generated_files: {e}")
        return []
...
def get_generated_file_by_name(name):
    ...
    """
    try:
        app.logger.info(f"Fetching generated file by name: {name}")
        ...
        conn.close()
        app.logger.debug(f"Generated file fetched: {row}")
    
        return row
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during get_generated_file_by_name: {e}")
        return None
    except Exception as e:
        app.logger.error(f"Unexpected error during get_generated_file_by_name: {e}")
        return None
...
def delete_generated_file(filename):
    ...
    """
    try:
        app.logger.info(f"Attempting to delete generated file: {filename}")
        ...
        conn.commit()
        app.logger.info(f"File '{filename}' deleted from database.")
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during delete_generated_file: {e}")
    except Exception as e:
        app.logger.error(f"Unexpected error during delete_generated_file: {e}")
    finally:
        conn.close()
        app.logger.debug("Database connection closed after delete_generated_file.")
    ...
    try:
        if os.path.exists(file_path):
            os.remove(file_path)
            app.logger.info(f"File '{filename}' deleted from filesystem.")
        else:
            app.logger.warning(f"File '{filename}' not found in filesystem.")
    except Exception as e:
        app.logger.error(f"Error deleting file '{filename}' from filesystem: {e}")
...
def delete_all_generated_files():
    ...
    """
    try:
        app.logger.info("Attempting to delete all generated files.")
        conn = sqlite3.connect(FILES_DB)  # Connect to the generated_files database
        cursor = conn.cursor()
        ...
        conn.commit()
        app.logger.info("All files deleted from database.")
    except sqlite3.Error as e:
        app.logger.error(f"SQLite error during delete_all_generated_files: {e}")
    except Exception as e:
        app.logger.error(f"Unexpected error during delete_all_generated_files: {e}")
    finally:
        conn.close()
        app.logger.debug("Database connection closed after delete_all_generated_files.")
    ...
    try:
        for filename in os.listdir(GENERATED_FILES_DIR):
            ...
            if os.path.isfile(file_path):
                ...
                app.logger.warning(f"File '{filename}' is not a regular file and was skipped.")
    except FileNotFoundError:
        app.logger.warning(f"File folder not found: {GENERATED_FILES_DIR}")
    except Exception as e:
        app.logger.error(f"Error deleting files from filesystem: {e}")
